...
-spec run_services(knm_numbers:collection()) -> knm_numbers:collection().
...
-spec maybe_dry_run_services(knm_numbers:collection()) -> knm_numbers:collection().
maybe_dry_run_services(T=#{todo := Numbers, options := Options}) ->
    case knm_number_options:crossbar(Options) of
        'undefined' -> knm_numbers:ok(Numbers, T);
        CrossbarOptions -> dry_run_services(T, CrossbarOptions)
    end.

-spec dry_run_services(knm_numbers:collection(), kz_term:proplist()) -> knm_numbers:collection().
dry_run_services(T=#{todo := Numbers}, CrossbarOptions) ->
    ...
    case kz_services_invoices:has_changes(Quotes) of
        'false' -> knm_numbers:ok(Numbers, T);
        'true' ->
            JObj = kz_services_invoices:public_json(Quotes),
            knm_numbers:ok(Numbers, T#{quotes => JObj})
    end.
